...
pub fn ligesis_commit<F: PrimeField>(
    ...
    let (com_mat_h, mat_h_advice) =
        chunked_batch_commit(deepfold_prover_param, &[mat_h_poly])?;
    end_timer!(timer);
...
pub fn ligesis_d_commit<F: PrimeField>(
    ...
    let mat_h_local: Vec<F> = if Net::am_master() {
        let all_mat_h_flat = all_mat_h_flat.ok_or(PCSError::UnexpectedNone("all_mat_h_flat".into()))?;
    ...
    // d_chunked_batch_commit will use "large polynomial" path if local size >= base_mu
