...
pub struct LigeSISProof<F: PrimeField> {
    ...
    pub com_a_r6_proof: DeepFoldProof<F>,
    pub com_rs_a_r3_proof: DeepFoldProof<F>,
    pub com_rs_a_alpha3_proof: DeepFoldProof<F>,
...
impl<F: PrimeField> PolynomialCommitmentScheme<F> for LigeSISPCS<F> {
    ...
    fn setup(
        ...
        let prover_param = LigeSISProverParam{
            eta, s_lambda, mu, log_m, log_n, rs_len, c, rs, mat_a, com_mat_a_advice, deepfold_prover_param,
        };
        let verifier_param = LigeSISVerifierParam{
            eta, s_lambda, mu, log_m, log_n, rs_len, c, g, com_mat_a, com_mat_a_v_advice, deepfold_verifier_param, 
        };
    ...
    fn commit(
        ...
        // compute `H`
        let start = std::time::Instant::now();
        let mat_a_prime = mat_a.iter().map(
            |row| (0..eta * m / 8).map(
                |i| get_mat_a_byte_bucket(&row[i * 8..(i + 1) * 8].to_vec())
            ).collect::<Vec<_>>()
        ).collect::<Vec<_>>();
        let mat_h = mat_a_prime.iter().map(
            |row| (0..n * 2).map(
                |j| (0..m).map(
                    |i| mat_f_prime[i][j].into_bigint().to_bytes_le().iter().enumerate().map(
                        |(k, x)| row[eta * i / 8 + k][*x as usize]
                    ).sum::<F>()
                ).sum::<F>()
            ).collect::<Vec<_>>()
        ).collect::<Vec<_>>();
        ...
        Ok((LigeSISCommitment{com_mat_h}, LigeSISProverCommitmentAdvice{mat_f_prime, mat_h, com_mat_h_advice}))
    }
    ...
    fn open(
        ...
        let LigeSISProverCommitmentAdvice{mat_f_prime, mat_h, com_mat_h_advice} = advice;
        ...
        let (com_a, com_a_advice) = DeepFoldPCS::commit_given_alpha(deepfold_prover_param, &evals_to_arcpoly(&a), &deepfold_alpha0, transcript)?;
        ...
        let mat_bI = transposition(&I.iter().map(|&i| decompose_vector(&mat_f_prime_trans[i])).collect::<Vec<_>>());
        let bI_field = bool_vec_to_field_vec(&mat_bI.concat());
        let (com_bI, com_bI_advice) = DeepFoldPCS::commit_given_alpha(deepfold_prover_param, &evals_to_arcpoly(&bI_field), &deepfold_alpha0, transcript)?;
        ...
        let g = rs.get_generator();
        let (com_rs_a, com_rs_a_advice) = DeepFoldPCS::commit_given_alpha(deepfold_prover_param, &evals_to_arcpoly(&rs_a), &deepfold_alpha0, transcript)?;
        ...
        for i in (2..=log_rs_len).rev() {
            ...
            let w = (0..1 << (i - 1)).map(
                |z| F::ONE - alpha3[log_rs_len - i] + alpha3[log_rs_len - i] * 
                    (gi.pow([z]) * (F::ONE - b) + gi.pow([z + (1 << (i - 1))]) * b)
            ).collect::<Vec<_>>();
            let mut mat_g_check = VirtualPolynomial::new(i - 1);
            mat_g_check.add_mle_list([
                evals_to_arcpoly(&get_tensor(&x)),
                evals_to_arcpoly(&alpha3_mat_g[i - 1]),
                evals_to_arcpoly(&w),
            ], F::ONE).unwrap();
            let mat_g_check_proof = <PolyIOP<F> as SumCheck<F>>::prove(mat_g_check, transcript).unwrap();
        ...
        let eq_alpha2_a_bI = mat_mul(&vec![get_tensor(&alpha2)], &field_mat_mul_bool_mat(&mat_a, &mat_bI)).concat();
        ...
        */
        assert!(
            (0..s_lambda).all(
                |k| eq_alpha2_a_bI[k] == eq_alpha2_h[I[k]] && v_bI[k] == rs_a[I[k]]
            )
        );
        let lookup_proof = ();
        ...
        let com_a_r6_proof = DeepFoldPCS::open(deepfold_prover_param, &evals_to_arcpoly(&a), &com_a_advice, &r6, transcript)?;
        let com_rs_a_r3_proof = DeepFoldPCS::open(deepfold_prover_param, &evals_to_arcpoly(&rs_a), &com_rs_a_advice, &r3, transcript)?;
        let com_rs_a_alpha3_proof = DeepFoldPCS::open(deepfold_prover_param, &evals_to_arcpoly(&rs_a), &com_rs_a_advice, &alpha3, transcript)?;
    ...
    fn verify(
        ...
        // trim parameters
        let LigeSISVerifierParam{eta, s_lambda, mu, log_m, log_n, rs_len, c, g, com_mat_a, com_mat_a_v_advice, deepfold_verifier_param} = verifier_param.borrow().clone();
        ...
        // Step 2
        let com_a_v_advice = DeepFoldPCS::verifier_receive_commit_given_alpha(&deepfold_verifier_param, &com_a, &deepfold_alpha0, transcript)?;

        // Step 3
        let I = transcript.get_and_append_challenge_indices(b"I", s_lambda, 2 * n)?;

        // Step 4
        let com_bI_v_advice = DeepFoldPCS::verifier_receive_commit_given_alpha(&deepfold_verifier_param, &com_bI, &deepfold_alpha0, transcript)?;
        ...
        let bI_check_claim = <PolyIOP<F> as SumCheck<F>>::verify(bI_check_sum, &bI_check_proof, &VPAuxInfo{
            max_degree: 3, 
            num_variables: (m * eta * s_lambda).ilog2() as usize, 
            phantom: PhantomData::<F>::default()
        }, transcript).unwrap();
        let r1 = bI_check_proof.point.clone();
        let bI_r1 = DeepFoldPCS::compute_value_from_proof(&r1, &com_bI_proof0);
        ...
        // Step 7
        let com_rs_a_v_advice = DeepFoldPCS::verifier_receive_commit_given_alpha(&deepfold_verifier_param, &com_rs_a, &deepfold_alpha0, transcript)?;
        
        //Step 7.1 
        let rs_a_check_sum = <PolyIOP<F> as SumCheck<F>>::extract_sum(&rs_a_check_proof);
        let rs_a_check_claim = <PolyIOP<F> as SumCheck<F>>::verify(rs_a_check_sum, &rs_a_check_proof, &VPAuxInfo{
            max_degree: 2, 
            num_variables: log_n, 
            phantom: PhantomData::<F>::default()
        }, transcript).unwrap();
        let r6 = rs_a_check_proof.point.clone();
        let a_r6 = DeepFoldPCS::compute_value_from_proof(&r6, &com_a_r6_proof);
        if rs_a_check_sum != DeepFoldPCS::compute_value_from_proof(&alpha3, &com_rs_a_alpha3_proof) {
                return Ok(false);
            }
        ...
        for i in (2..=log_rs_len).rev() {
            ...
            let mat_g_check_claim = <PolyIOP<F> as SumCheck<F>>::verify(mat_g_check_sum, mat_g_check_proof, &VPAuxInfo{
                max_degree: 3, 
                num_variables: i - 1, 
                phantom: PhantomData::<F>::default()
            }, transcript).unwrap();
            let mut r = mat_g_check_proof.point.clone(); r.push(b);
            let v = (0..i).map(
                |k| F::ONE - r[k] + r[k] * g.pow([rs_len as u64 >> (i - k)])
            ).product::<F>();
            expected_eval = mat_g_check_claim.expected_evaluation / eval_mle_eq(&mat_g_check_proof.point, &x)
                                                                  / (F::ONE - alpha3[log_rs_len - i] + alpha3[log_rs_len - i] * v);
            cur_p = mat_g_check_proof.point.clone();
        }
        if expected_eval != F::ONE - alpha3[log_rs_len - 1] + alpha3[log_rs_len - 1] * (F::ONE - cur_p[0] + cur_p[0] * g.pow([rs_len as u64 >> 1])) {
            return Ok(false);
        }
        ...
        let alpha2_a_bI_r2_check_claim = <PolyIOP<F> as SumCheck<F>>::verify(alpha2_a_bI_r2_check_sum, &alpha2_a_bI_r2_check_proof, &VPAuxInfo{
            max_degree: 2, 
            num_variables: (m * eta).ilog2() as usize, 
            phantom: PhantomData::<F>::default()
        }, transcript).unwrap();
        let r4 = alpha2_a_bI_r2_check_proof.point.clone();
        let bI_r4_r2 = DeepFoldPCS::compute_value_from_proof(&vec![r2.clone(), r4.clone()].concat(), &com_bI_proof1);
        let mat_a_alpha2_r_4 = DeepFoldPCS::compute_value_from_proof(&vec![r4.clone(), alpha2.clone()].concat(), &com_mat_a_proof);
        if alpha2_a_bI_r2_check_claim.expected_evaluation !=
            bI_r4_r2 * mat_a_alpha2_r_4 {
                return Ok(false);
            }
        
        // Step 10
        let v_bI_r2_check_sum = <PolyIOP<F> as SumCheck<F>>::extract_sum(&v_bI_r2_check_proof);
        let v_bI_r2_check_claim = <PolyIOP<F> as SumCheck<F>>::verify(v_bI_r2_check_sum, &v_bI_r2_check_proof, &VPAuxInfo{
            max_degree: 2, 
            num_variables: (m * eta).ilog2() as usize, 
            phantom: PhantomData::<F>::default()
        }, transcript).unwrap();
        let r5 = v_bI_r2_check_proof.point.clone();
        let bI_r5_r2 = DeepFoldPCS::compute_value_from_proof(&vec![r2.clone(), r5.clone()].concat(), &com_bI_proof2);
        let (r5_0, r5_1) = (r5[..eta.ilog2() as usize].to_vec(), r5[eta.ilog2() as usize..].to_vec());
        ...
        if v_bI_r2_check_claim.expected_evaluation !=
            bI_r5_r2 * eval_mle_eq(&r5_1, &z1) * r5_0_p {
                return Ok(false);
            }
        ...
        // Step 11
        let coms = vec![
            &com_a, &com_a, &com_rs_a, &com_rs_a, 
            &com_mat_h, &com_mat_a, &com_bI, &com_bI,
        ];
        let points = vec![
            z2, r6, r3.clone(), alpha3.clone(),
            vec![r3.clone(), alpha2.clone()].concat(), 
            vec![r4.clone(), alpha2.clone()].concat(),
            r1, vec![r2.clone(), r4.clone()].concat(),
            vec![r2.clone(), r5.clone()].concat(),
        ];
        let proofs = vec![
            &com_a_proof, &com_a_r6_proof, &com_rs_a_r3_proof, 
            &com_rs_a_alpha3_proof, &com_h_proof, &com_mat_a_proof, 
            &com_bI_proof0, &com_bI_proof1, &com_bI_proof2,
        ];
        let values = points.iter().zip(proofs.iter())
            .map(|(point, proof)| DeepFoldPCS::compute_value_from_proof(point, proof))
            .collect::<Vec<_>>();
        let advices = vec![
            &com_a_v_advice, &com_a_v_advice, &com_rs_a_v_advice, 
            &com_rs_a_v_advice, &com_mat_h_v_advice, &com_mat_a_v_advice, 
            &com_bI_v_advice, &com_bI_v_advice, &com_bI_v_advice,
        ];
